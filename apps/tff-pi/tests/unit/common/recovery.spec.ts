...
describe("recovery", () => {
	...
	describe("summarizeInput", () => {
		...
		it("returns the bash command, truncating at 80 chars", () => {
			expect(summarizeInput("bash", { command: "pnpm run test" })).toBe("pnpm run test");
		...
		it("strips ANSI escape sequences from bash commands", () => {
			const cmd = "pnpm test \x1b[31mFAIL\x1b[0m";
			expect(summarizeInput("bash", { command: cmd })).toBe("pnpm test FAIL");
		});
	...
	describe("formatRecoveryBriefing", () => {
		it("renders a Recent tool calls section when evidence has entries", () => {
			...
			expect(briefing).toContain("pnpm run test");
