...
describe("parseVerificationClaims", () => {
	...
	it("parses backticked command with 'all pass' keyword (expectedExit=0)", () => {
		const md = "Ran `pnpm run test` — all 677 tests pass.";
		const claims = parseVerificationClaims(md);
		expect(claims).toHaveLength(1);
		expect(claims[0]).toMatchObject({ command: "pnpm run test", expectedExit: 0 });
	});

	it("parses backticked command with explicit 'exit 1'", () => {
		const md = "`pnpm run test` exit 1";
		const claims = parseVerificationClaims(md);
		expect(claims).toHaveLength(1);
		expect(claims[0]).toMatchObject({ command: "pnpm run test", expectedExit: 1 });
	});
	...
	it("parses fenced-block '$ <cmd>' with clean output as expectedExit=0", () => {
		...
		const claims = parseVerificationClaims(md);
		expect(claims).toHaveLength(1);
		expect(claims[0]).toMatchObject({ command: "pnpm run typecheck", expectedExit: 0 });
	});
	...
	it("fenced block: blank lines in output don't terminate the failure scan", () => {
		// Real pnpm/jest output often has blank lines between sections,
		// with the failure summary only visible AFTER the blank line.
		const md = "```\n$ pnpm test\n\nerror: assertion failed\n```\n";
		const claims = parseVerificationClaims(md);
		expect(claims).toHaveLength(1);
		expect(claims[0]).toMatchObject({ command: "pnpm test", expectedExit: 1 });
	});

	it("short claim 'pnpm' does not match 'pnpm run test' (exploit hardening)", () => {
		// Before: substring-matcher allowed claim 'pnpm' to silently match any captured pnpm call.
		...
		expect(claims).toHaveLength(1);
		expect(claims[0]?.command).toBe("pnpm");
	});

	it("cross-pattern dedup: same command in fenced block AND inline verdict only counted once per (command, expectedExit)", () => {
		const md = "Ran `pnpm run test` — all pass\n\n```\n$ pnpm run test\nok\n```";
		const claims = parseVerificationClaims(md);
		// Both patterns produce (command: "pnpm run test", expectedExit: 0).
...
describe("auditVerification", () => {
	...
	it("verdict=match when claim says pass and event has isError=false", () => {
		seedBashEvent("pnpm run test", false, "2026-04-13T12:00:01Z");
		const md = "Ran `pnpm run test` — all pass";
	...
	it("verdict=mismatch when claim says pass and event has isError=true", () => {
		seedBashEvent("pnpm run test", true, "2026-04-13T12:00:01Z");
		const md = "Ran `pnpm run test` — all pass";
		const report = auditVerification(root, SLICE_LABEL, md);
	...
	it("matcher picks the most recent event by startedAt when multiple candidates exist", () => {
		seedBashEvent("pnpm run test", true, "2026-04-13T12:00:01Z");
		seedBashEvent("pnpm run test", false, "2026-04-13T12:05:00Z");
		const md = "Ran `pnpm run test` — all pass";
		const report = auditVerification(root, SLICE_LABEL, md);
		expect(report.findings[0]?.verdict).toBe("match");
	...
	it("matcher: single-token claim 'pnpm' does NOT match captured 'pnpm run test'", () => {
		seedBashEvent("pnpm run test", true, "2026-04-13T12:00:01Z");
		const md = "Ran `pnpm` — all pass";
		const report = auditVerification(root, SLICE_LABEL, md);
		// Short-claim matcher returns unverifiable (no token-prefix match with "pnpm run test").
		expect(report.findings[0]?.verdict).toBe("unverifiable");
		expect(report.summary.unverifiable).toBe(1);
	});

	it("matcher: token-prefix claim 'pnpm run test' DOES match captured 'pnpm run test --watch' (paraphrase allowed)", () => {
		seedBashEvent("pnpm run test --watch", false, "2026-04-13T12:00:01Z");
		const md = "Ran `pnpm run test` — all pass";
		const report = auditVerification(root, SLICE_LABEL, md);
		expect(report.findings[0]?.verdict).toBe("match");
	});
	...
	it("formatAuditReport produces markdown with all three sections when findings exist", () => {
		seedBashEvent("pnpm run test", true, "2026-04-13T12:00:01Z");
		seedBashEvent("pnpm run typecheck", false, "2026-04-13T12:00:02Z");
		const md =
			"Ran `pnpm run test` — all pass\nRan `pnpm run typecheck` — all pass\nRan `rg nope src/` — all pass";
		const report = auditVerification(root, SLICE_LABEL, md);
...
describe("auditVerificationAgainstCapture", () => {
	it("returns hasMismatches=false when all claimed commands succeeded", () => {
		const vMd = "Ran `pnpm test` — all pass.";
