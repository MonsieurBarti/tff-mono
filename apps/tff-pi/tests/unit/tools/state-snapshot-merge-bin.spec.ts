...
const BIN = join(process.cwd(), "dist/tools/state-snapshot-merge.js");
...
describe("state-snapshot-merge bin", () => {
	...
	it("exits 0 and writes merged snapshot on clean merge", () => {
		...
		const result = spawnSync("node", [BIN, base, ours, theirs, "state-snapshot.json"], {
			encoding: "utf-8",
		});
	...
	it("exits 1 and prints conflicts on unresolvable merge", () => {
		...
		const result = spawnSync("node", [BIN, O, A, B, "state-snapshot.json"], { encoding: "utf-8" });
	...
	it("exits 1 with a schema-mismatch message on bad schemaVersion", () => {
		...
		const result = spawnSync("node", [BIN, tooOld, ok, ok, "state-snapshot.json"], {
			encoding: "utf-8",
		});
		expect(result.status).toBe(1);
	...
	it("exits 1 with a JSON-parse error on malformed input", () => {
		...
		const ok = write("A", makeSnap());
		const result = spawnSync("node", [BIN, bad, ok, ok, "state-snapshot.json"], {
			encoding: "utf-8",
		});
		expect(result.status).toBe(1);
