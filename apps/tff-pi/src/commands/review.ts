...
import type { ValidateResult } from "../common/dto.js";
...
export function validateReview(
	...
	if (slice.status !== "verifying" && slice.status !== "reviewing") {
		return {
			valid: false,
			error: `Cannot review: slice is in '${slice.status}' status (expected 'verifying' or 'reviewing')`,
		};
	}
	return assertPhasePreconditions(db, projectRoot, sliceId, "review");
}
...
export async function runReview(
	...
	const validation = validateReview(database, slice.id, root);
	if (!validation.valid) {
	...
	if (uiCtx?.hasUI)
		uiCtx.ui.notify(
			`Starting review phase for ${sliceLabel(milestone.number, slice.number)}...`,
			"info",
		);
	await runHeavyPhase(ctx, "review", mod, phaseCtx);
}
