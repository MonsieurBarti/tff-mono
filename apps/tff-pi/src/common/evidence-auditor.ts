	...
	while (blockMatch) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			for (let j = i + 1; j < lines.length; j++) {
				...
				// normal (pnpm test, jest, etc. emit blank lines before the failure
...
function commandsOverlap(claim: string, actual: string): boolean {
	...
	// Single-token claims (e.g., "npm") are too generic for fuzzy match.
	...
	// "pnpm run test" ↔ "pnpm run test --watch" while rejecting
	// "pnpm" matching "pnpm run test".
