...
After the tool returns successfully, STOP. Do not call any other tools. The system handles plan review automatically — do NOT call `plannotator_submit_plan`, `plannotator_annotate`, or any plannotator\_\* tool. TFF emits the review request on an event bus; plannotator opens its UI modal; the tool return only resolves after the user approves in the UI.
