...
export const STATUS_COLOR: Record<
	ProgressStatus,
	"dim" | "accent" | "success" | "error" | "warning"
> = {
	pending: "dim",
	active: "accent",
	done: "success",
...
export interface UI {
	...
	optionUnselected(
		...
		label: string,
		description: string,
		opts?: { isCommitted?: boolean; isFocusDimmed?: boolean },
	): string[];
	...
	return {
		...
		optionSelected: (num, label, description, isCommitted = false) => {
			...
			return [
				...
				...wrapIndented(
					`${INDENT.description}${theme.fg("muted", description)}`,
					INDENT.description,
				),
			];
		},
		...
		optionUnselected: (num, label, description, opts = {}) => {
			...
			return [
				...
				...wrapIndented(
					`${INDENT.description}${theme.fg(descColor, description)}`,
					INDENT.description,
				),
			];
		},
		...
		checkboxSelected: (label, description, isChecked) => {
			...
			return [
				add(
					`${INDENT.option}${theme.fg("accent", GLYPH.cursor)} ${box} ${theme.fg("accent", label)}`,
				),
				...wrapIndented(
					`${INDENT.description}${theme.fg("muted", description)}`,
					INDENT.description,
		...
		doneSelected: () => [
			add(
				`${INDENT.option}${theme.fg("accent", INDENT.cursor)}${theme.bold(theme.fg("accent", "Done"))}`,
			),
		],
		...
		actionSelected: (num, label, description, tag) => {
			...
			const lines = [
				add(
					`${INDENT.option}${theme.fg("accent", GLYPH.cursor)} ${theme.fg("accent", `${num}. ${label}`)}${tagStr}`,
				),
			];
			if (description)
		...
		progressItem: (label, status, opts = {}) => {
			...
			const labelColor =
				status === "done" ? "muted" : status === "pending" || status === "skipped" ? "dim" : "text";
