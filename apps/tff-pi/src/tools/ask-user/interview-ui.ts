...
export async function showInterviewRound(
	...
	return ctx.ui.custom<RoundResult>((tui: TUI, theme: Theme, _kb, done) => {
		...
		function isMultiSelect(qIdx: number): boolean {
			return !!(questions[qIdx] as Question).allowMultiple;
		}

		function totalOpts(qIdx: number): number {
			return (questions[qIdx] as Question).options.length + 1;
		}

		function noneOrDoneIdx(qIdx: number): number {
			return (questions[qIdx] as Question).options.length;
		}

		function saveEditorToState() {
			(states[currentIdx] as QuestionState).notes = getEditor().getExpandedText().trim();
		}

		function loadStateToEditor() {
			getEditor().setText((states[currentIdx] as QuestionState).notes);
		}

		function isQuestionAnswered(idx: number): boolean {
			if (isMultiSelect(idx)) return (states[idx] as QuestionState).checkedIndices.size > 0;
			return (states[idx] as QuestionState).committedIndex !== null;
		}
		...
		function switchQuestion(newIdx: number) {
			...
			currentIdx = newIdx;
			loadStateToEditor();
			const cur = states[currentIdx] as QuestionState;
			focusNotes = cur.notesVisible && cur.notes.length > 0;
			refresh();
		}
		...
		function buildResult(): RoundResult {
			...
			for (let i = 0; i < questions.length; i++) {
				...
				const st = states[i] as QuestionState;
				const notes = st.notes.trim();

				if (isMultiSelect(i)) {
					const sorted = Array.from(st.checkedIndices).sort((a, b) => a - b);
					const selected = sorted.map((idx) => (q.options[idx] as QuestionOption).label);
					...
					if (st.committedIndex !== null) {
						...
						if (idx < q.options.length) selected = (q.options[idx] as QuestionOption).label;
		...
		function goNextOrSubmit() {
			...
			if (!isMultiSelect(currentIdx)) {
				cur.committedIndex = cur.cursorIndex;
			}
			...
			if (
				!isMultiSelect(currentIdx) &&
				cur.cursorIndex === noneOrDoneIdx(currentIdx) &&
				!cur.notes &&
				!cur.notesVisible
			) {
				cur.notesVisible = true;
				focusNotes = true;
				loadStateToEditor();
				refresh();
				return;
		...
		function handleInput(data: string) {
			...
			const st = states[currentIdx] as QuestionState;
		...
		function renderReviewScreen(width: number): string[] {
			...
			push(
				ui.bar(),
				ui.blank(),
				ui.header(`  ${opts.reviewHeadline ?? "Review your answers"}`),
				ui.blank(),
			);
			...
			for (let i = 0; i < questions.length; i++) {
				...
				if (isMultiSelect(i)) {
					const selected = Array.from(st.checkedIndices)
						.sort((a, b) => a - b)
						.map((idx) => (q.options[idx] as QuestionOption).label);
					...
					if (st.committedIndex !== null && st.committedIndex < q.options.length) {
						label = (q.options[st.committedIndex] as QuestionOption).label;
					}
		...
		function render(width: number): string[] {
			...
			const q = questions[currentIdx] as Question;
			const st = states[currentIdx] as QuestionState;
			const multiSel = isMultiSelect(currentIdx);
			...
			if (isMultiQuestion) {
				...
				const answeredSet = new Set(
					questions.map((_, i) => i).filter((i) => isQuestionAnswered(i)),
				);
				push(
					ui.questionTabs(
			...
			for (let i = 0; i < q.options.length; i++) {
				const opt = q.options[i] as QuestionOption;
				...
				if (multiSel) {
					...
					if (isCursor && !focusNotes) {
						...
						push(
							ui.optionUnselected(i + 1, opt.label, opt.description, {
								isCommitted,
			...
			if (multiSel) {
				...
				if (ndCursor && !focusNotes) {
					...
					push(
						ui.slotUnselected(OTHER_OPTION_LABEL, OTHER_OPTION_DESCRIPTION, {
