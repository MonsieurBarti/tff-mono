...
#   3. pnpm --filter @the-forge-flow/tff-cc... build && bash scripts/sync-release-branch.sh
...
#   pnpm --filter @the-forge-flow/tff-cc... build
...
# release-please cuts a release, so a force-push here always reflects a
# tagged version.
...
if [ ! -f dist/cli/index.js ]; then
  echo "error: dist/cli/index.js missing — run 'pnpm --filter @the-forge-flow/tff-cc... build' first" >&2
  exit 1
fi
...
# plugin/<sub> is a committed symlink. We intentionally resolve
...
for sub in agents bin commands hooks references skills workflows; do
...
# --- 5a. Build manifest verification script ---------------------------------

if [ -f scripts/verify-build-manifest.sh ]; then
  echo "Copying verify-build-manifest.sh..."
  cp scripts/verify-build-manifest.sh "$RELEASE_DIR/"
fi
...
# In the monorepo, workflows live at the repository root, so we copy from
# ../../.github/workflows/ relative to apps/tff-cc/.
WORKFLOWS_SRC="../../.github/workflows"
if [ ! -d "$WORKFLOWS_SRC" ]; then
  echo "error: $WORKFLOWS_SRC missing — release gating cannot work without it" >&2
  exit 1
fi
echo "Copying .github/workflows/ into release tree..."
mkdir -p "$RELEASE_DIR/.github/workflows"
cp -r "$WORKFLOWS_SRC/." "$RELEASE_DIR/.github/workflows/"
