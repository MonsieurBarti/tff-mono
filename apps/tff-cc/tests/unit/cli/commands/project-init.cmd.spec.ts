...
describe("project:init — .tff/ auto-creation", () => {
	...
	it("creates .tff/ symlink and project-id under TFF_CC_HOME (cwd untouched)", async () => {
		expect(existsSync(path.join(homeDir, ".tff"))).toBe(false);
		...
		expect(existsSync(path.join(homeDir, ".tff"))).toBe(true);
