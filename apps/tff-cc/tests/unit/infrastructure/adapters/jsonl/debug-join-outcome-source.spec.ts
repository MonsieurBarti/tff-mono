...
describe("DebugJoinOutcomeSource", () => {
	...
	it("emits one outcome per decisionId when debug follows ship on same slice", async () => {
		...
		expect(collected[0].decisionId).toBe("00000000-0000-4000-8000-000000000001");
	});
	...
	it("attributes debug to most recent prior ship (slice-bounded, next ship supersedes)", async () => {
		...
		const ids = [];
		for await (const o of src.readOutcomes({})) ids.push(o.decisionId);
		expect(ids).toEqual(["00000000-0000-4000-8000-0000000000b1"]);
	});

	it("is deterministic across identical inputs (idempotent outcomeIds)", async () => {
		await writeLine(routingPath, {
			kind: "route",
			timestamp: "2026-04-19T09:00:00.000Z",
			workflow_id: "tff:ship",
		...
		const a = await run();
		const b = await run();
		expect(a.map((x) => x.outcomeId)).toEqual(b.map((x) => x.outcomeId));
	});

	it("filter by decisionId returns only matching outcome", async () => {
		await writeLine(routingPath, {
			kind: "route",
			timestamp: "2026-04-19T09:00:00.000Z",
			workflow_id: "tff:ship",
		...
		for await (const o of src.readOutcomes({ decision_id: "00000000-0000-4000-8000-000000000011" }))
			collected.push(o);
		expect(collected).toHaveLength(1);
		expect(collected[0].decisionId).toBe("00000000-0000-4000-8000-000000000011");
	});
	...
	it("workflowId is captured from the route event, not hardcoded", async () => {
		await writeLine(routingPath, {
			kind: "route",
			timestamp: "2026-04-19T09:00:00.000Z",
		...
		for await (const o of src.readOutcomes({})) collected.push(o);
		expect(collected).toHaveLength(1);
		expect(collected[0].workflowId).toBe("custom:workflow");
	});

	it("outcomeId is a valid UUIDv5 (version nibble is 5)", async () => {
		await writeLine(routingPath, {
			kind: "route",
			timestamp: "2026-04-19T09:00:00.000Z",
			workflow_id: "tff:ship",
		...
		expect(outcome.outcomeId.charAt(14)).toBe("5");
	});
});
