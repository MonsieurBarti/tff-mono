...
observation enabled ∈ `{{settings-path}}` ∧ `{{project-dir}}/observations/sessions.jsonl` ∃
LOAD @skills/skill-authoring/SKILL.md

## Settings

Read `{{settings-path}}` → `auto-learn.weights`.
...
4. DISPLAY `{{project-dir}}/observations/candidates.jsonl`: ranked candidates w/ scores + sequences
   - ∅ candidates above threshold → inform user, suggest lower threshold ∨ more observations
5. NEXT: suggest `{{command-prefix}}suggest-skills` ∨ `{{command-prefix}}create-skill`
