...
1. **Workflows stay small.** Orchestrator's job:
   ...
   - Spawn right agent for job (via {{spawn-agent}})
...
5. **State transitions go through tff-tools.** Orchestrator calls `tff-tools` for all state changes. Agents don't transition state directly.
...
2. **Spawn** — dispatch agent via {{spawn-agent}}
