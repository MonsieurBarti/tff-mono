...
The verify phase runs as a single subagent dispatch. You do NOT author completion; the host's result hook runs the verify finalizer, which ingests the artifacts you write and completes the phase on success.
...
4. Write `{{project-dir}}/artifacts/VERIFICATION.md`: AC checklist with `- [x]` / `- [ ]`, test command + counts, on fail the task(s) to rework.
5. Write `{{project-dir}}/artifacts/PR.md`: concise reviewer-facing description (≤20 lines), uncompressed.
