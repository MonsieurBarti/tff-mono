---
name: judge
description: Grade routing decisions for a closed slice
version: "1.0.0"
...
```bash
{{command-tool}} routing:judge-prepare --slice <slice-label>
```
...
Use the {{spawn-agent}} with `subagent_type: "tff-outcome-judge"`. When the agent returns, verify the verdicts file exists and is non-empty. If the agent reported failure or the file is missing, surface the error and stop.
...
```bash
{{command-tool}} routing:judge-record --slice <slice-label> --verdicts-path "$VERDICTS_PATH" \
