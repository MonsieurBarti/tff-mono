...
7. CLOSE + CLEANUP:
   ...
   - DRAIN routing judgment (run inline, before branch deletion):
     ...
     - On any error in this DRAIN block: surface the error, leave the pending row, stop the cleanup. The user can retry via `{{command-prefix}}judge --slice-id <slice-id>` later.
